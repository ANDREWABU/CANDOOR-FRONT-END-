...
class YourJourney extends React.Component {
    ...
    state = {
        modelType: '', educationId: '', modal: '', headline: '', currentWorkDisabled: false, currentEducationDisabled: false, workEndDateDisabled: false, educationEndDateDisabled: false,
        ...
        dream_industries: [], dream_roles: [], excited_topics: [],
        errors: {
            headline: '', highest_degree: '', school: '', degree: '', fields_of_study: '', graduation_year: '', loading: false, excited_topics_error: '', dream_industries_error: '', dream_roles_error: '', headlineError: '', highest_degreeError: '', schoolError: '', degreeError: '', fields_of_studyError: '', graduation_yearError: '', loadingError: false, employment_type_checkedError: '', employment_otherError: '',
            years_work_experience: '', comfort_networking_at_signup: '', title: '', company: '', industry: '', role: '', employment_type: '', ask_me_about: '', is_current: '', start_date: '', end_date: '', loading: false, education_emtpy_error: ''
        },
        data: {
            dropdownData: {
                dropdownWorkRoles: [],
                dropdownWorkIndustries: [],
                reasonsForJoining: [],
            },
            educationData: {
    ...
    continue = async () => {
        ...
        const { excited_topics, dream_industries, dream_roles } = this.state;
        let { dream_industries_error, dream_roles_error, excited_topics_error, education_emtpy_error } = '';

        if (excited_topics.length == 0) {
            excited_topics_error = 'Please select at least one'
        }
        if (dream_industries.length == 0) {
            dream_industries_error = 'Please select at least one'
        }
        if (dream_roles.length == 0) {
            dream_roles_error = 'Please select at least one'
        }

        if (this.state.data.educationData.education_experiences.length == 0) {
            education_emtpy_error = 'Please enter your education'
        }

        this.setState(prevState => ({
            errors: {
                ...prevState.errors,
                dream_industries_error: dream_industries_error,
                dream_roles_error: dream_roles_error,
                excited_topics_error: excited_topics_error,
                education_emtpy_error: education_emtpy_error
            }
        }))
        if (dream_industries_error || dream_roles_error || excited_topics_error || education_emtpy_error) {
            return true;
        }
        ...
        const data = {
            excited_topics, dream_industries, dream_roles
        }
    ...
    getDropdownData = async () => {
        ...
        let response = await ApiRequest.getRequest('/api/get-work-journey-drop-data');
        response = response.data;

        let data = this.state.data.dropdownData;
        if (response.result) {


            data.dropdownWorkRoles = this.selectOptions(response.result.work_roles);
            data.dropdownWorkIndustries = this.selectOptions(response.result.industries);
            data.reasonsForJoining = this.selectOptions(response.result.signup_joining_reasons);
    ...
    getWorkData = async () => {
        ...
        let selectedWorkRoles = [];
        let selectedWorkIndustries = [];
        let selectedreasonsForJoining = [];
        ...
        if (response.result) {
            ...
            selectedWorkRoles = response.result.dream_roles ? JSON.parse(response.result.dream_roles) : [];
            selectedreasonsForJoining = response.result.services_requested ? JSON.parse(response.result.services_requested) : [];
            selectedWorkIndustries = response.result.dream_industries ? JSON.parse(response.result.dream_industries) : [];


            this.setState(prevState => ({
                headline: headline,
                excited_topics: selectedreasonsForJoining,
                dream_industries: selectedWorkIndustries,
                dream_roles: selectedWorkRoles,
    ...
    changeStep(position = 'next') {
        if (position == 'back') {
            history.push('/advisee/signupwizard/yourBackground')
        }
        else {
            history.push('/advisee/signupwizard/yourProfile')
        }
    }
        ...
        return (
            <div className={`row mar-width-0   mt-80 NewStepThird__Wrapper`}>
                ...
                <div className="col-md-12 form-inputs- NewStepThird-p1">
                    <label htmlFor=""><b>Add your education experience below</b> Please do not include high schools or secondary schools.</label>
                </div>
                ...
                <div className="col-md-12">
                    <div className="row mar-width-0 educationPanel">
                        ...
                        <span className="error">{this.state.errors.education_emtpy_error}</span>
                    </div>
                </div>

                <div className="col-md-12 form-inputs- NewStepThird-p1 margintop">
                    <label htmlFor=""><b>Add your work experience below</b> If you have no full-time work experience, please include internships and part-time roles, if any.</label>
                </div>
                ...
                <div className="col-md-12 form-inputs- margintop">
                    <label htmlFor="">I am interested in the following industries:</label>
                    <Select classNamePrefix="react-select-custom" isMulti options={this.state.data.dropdownData.dropdownWorkIndustries} name="dream_industries" onChange={this.handleChange} value={this.state.dream_industries} placeholder="Select up to 3 that apply ..." />
                    <span className="error">{this.state.errors.dream_industries_error}</span>
                </div>
                <div className="col-md-12 form-inputs-">
                    <label htmlFor="">I am interested in the following roles:</label>
                    <Select classNamePrefix="react-select-custom" isMulti options={this.state.data.dropdownData.dropdownWorkRoles} name="dream_roles" onChange={this.handleChange} value={this.state.dream_roles} placeholder="Select up to 3 that apply ..." />
                    <span className="error">{this.state.errors.dream_roles_error}</span>
                </div>
                <div className="col-md-12 form-inputs-">
                    <label htmlFor="" className="label-custom-">I am joining Candoor because I need support with ...</label>
                    <Select classNamePrefix="react-select-custom" isMulti options={this.state.data.dropdownData.reasonsForJoining} name="excited_topics" onChange={this.handleChange} value={this.state.excited_topics} placeholder="Select up to 3 that apply ..." />
                    <span className="error">{this.state.errors.excited_topics_error}</span>
                </div>
