...
class YourBackground extends React.Component {
    ...
    getMainData = async () => {
        ...
        if (response.result) {
            ...
            response.result.identity.forEach((item) => {
                socioeconomicstatus.push(item.socio_economic_status)
    ...
    continue = async () => {
        ...
        if (!time_zone) {
            time_zoneError = 'Please enter your timezone.';
        }
        if (!gender) {
            genderError = 'Please enter your gender identity.';
        }
        if (race.length == 0) {
        ...
        const data = {
            country: country, city, state, time_zone, gender, race, belongs_to
        }
        let response = await ApiRequest.postRequestAdvisor('add-user-background', data);
    ...
    changeStep(position = 'next') {
        if (position == 'back') {
            history.push('/advisor/signupwizard/journey')
        }
        else {
            history.push('/advisor/signupwizard/profile')
        }
    }
        ...
        return (
            <div className={`row mar-width-0 mt-80  fifthStep NewStepThird__Wrapper`}>
                ...
                <div className="col-md-12 text-center wizardHeading">
                    ...
                    <p>
                        We welcome Advisors of all identities and backgrounds. This information helps us measure the diversity of our user base and recommend Advisors to Advisees with simillar lived experiences.
                    </p>
                </div>
